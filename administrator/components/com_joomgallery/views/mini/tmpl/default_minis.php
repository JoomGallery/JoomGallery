...
<?php if($row->thumb_src): ?>
      <a href="javascript:if(typeof window.parent.joom_selectimage == 'function'){window.parent.joom_selectimage(<?php echo $row->id; ?>, '<?php echo str_replace("'", "\'", $this->escape(stripslashes($row->imgtitle))); ?>', '<?php echo $this->object; ?>', '<?php echo $row->imgthumbname; ?>');}else{insertJoomPluWithId('<?php echo $row->id; ?>', '<?php echo $this->e_name; ?>');}" rel="<?php echo $row->overlib; ?>" class="hasMiniTip">
