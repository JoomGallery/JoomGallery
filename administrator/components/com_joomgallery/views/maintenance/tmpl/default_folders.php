    ...
    <table class="table table-striped" id="folderList">
      <thead>
        <tr>
          ...
          <th>
            <?php echo JHtml::_('grid.sort', 'COM_JOOMGALLERY_MAIMAN_OF_FULLPATH', 'a.fullpath', $listDirn, $listOrder); ?>
          </th>
        ...
        <tr class="row<?php echo $k; ?>">
          ...
          <!--<td class="center hidden-phone">
            <?php /*if($row->type != 'unknown'):*/
                    echo JText::_('COM_JOOMGALLERY_TYPE_'.strtoupper($row->type));
                  /*else:
          ...
          <td class="center hidden-phone">
<?php       if($row->refid): ?>
            <span title="<?php echo JText::_('COM_JOOMGALLERY_MAIMAN_OF_SHOW_CATEGORY_DETAILS'); ?>" class="hasTooltip">
              <a href="index.php?option=<?php echo _JOOM_OPTION; ?>&amp;controller=categories&amp;task=edit&amp;cid=<?php echo $row->refid; ?>">
                <?php echo $row->title; ?>
              </a>
            </span>
            <?php echo $this->correct('addorphanedfolder', $row->id, JText::_('COM_JOOMGALLERY_MAIMAN_OF_ADD_ORPHANED_FOLDER_TO_CATEGORY'), false, '&amp;tab=folders'); ?>
<?php       else: ?>
            <?php echo $this->cross('COM_JOOMGALLERY_MAIMAN_NO_PROPOSAL'); ?>
            <?php echo $this->correct('deleteorphanedfolder', 0, JText::_('COM_JOOMGALLERY_MAIMAN_OF_DELETE_THIS_ORPHANED_FOLDER'), 'javascript:listItemTask(\'cb'.$i.'\', \'deleteorphanedfolder\');', '&amp;tab=folders'); ?>
<?php       endif; ?>
          </td>
